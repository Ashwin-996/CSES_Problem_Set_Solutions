...
{
	fact[0] = 1;
	FOR(i,1,nax) fact[i] = mod_mul(fact[i-1], i, MOD);
}
...
int main()
{
	...
	if(flag) cout<< 0 <<nl;
	else cout<< mod_sub(mod_div_prime(fact[n-(l+r)], mod_mul(fact[(n/2)-l], fact[(n/2)-r], MOD), MOD), mod_div_prime(fact[n-(l+r)], mod_mul(fact[(n/2)-l-1], fact[(n/2)-r+1], MOD), MOD), MOD) <<nl;
