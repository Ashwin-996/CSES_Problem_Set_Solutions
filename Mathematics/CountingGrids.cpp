...
int main()
{
	...
	{
		ll x = (n-1)/2;
		expo = mod_mul(x, x+1, MOD-1);
	}
	else 
	{
		ll x = n/2;
		expo = mod_sub(mod_mul(x, n+1, MOD-1), mod_mul(x, x+1, MOD-1), MOD-1);
	}
 
	ll ans = 0;
	for(ll i=0; i<4LL; i++) add_self(ans, mod_pow(2LL, mod_mul(expo, (ll)__gcd(i,4LL), MOD-1), MOD)); 
	ans = mod_mul(mod_div_prime(ans, 4LL, MOD), ((n%2) ? 2LL : 1LL), MOD);
	cout<< ans <<nl;
