...
vll2 mat_op(vll2 mat1, vll2 mat2)
{
	vll2 mat(n, vll(n, INF));
	FOR(i,0,n)
	{
		FOR(j,0,n)
		{
			FOR(k,0,n)
			{
				if(mat1[i][k] == INF || mat2[k][j] == INF) continue;
				min_self(mat[i][j], mat1[i][k] + mat2[k][j]);
			}
		}
	}
...
vll2 mat_pow(vll2 mat, ll k)
{
	if(k==1) return mat;
	vll2 mat_temp = mat_pow(mat, k/2);
	mat_temp = mat_op(mat_temp, mat_temp);
	
	if(k%2) mat_temp = mat_op(mat_temp, mat);
...
int main()
{
	...
	while(t--)
	{
		...
		while(m--)
		{
			...
			min_self(mat[a-1][b-1], c);
		}
		mat = mat_pow(mat, k);
 
		cout<< ((mat[0][n-1]==INF) ? -1 : mat[0][n-1]) <<nl;
	}
