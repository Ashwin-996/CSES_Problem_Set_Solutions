...
void update(node *n)
{
	n->res.pref = max(n->left->res.pref, n->left->res.summ + n->right->res.pref);
	n->res.suff = max(n->right->res.suff, n->right->res.summ + n->left->res.suff);
	n->res.summ = n->left->res.summ + n->right->res.summ;
	n->res.ans = max(n->left->res.ans, max(n->right->res.ans, n->left->res.suff + n->right->res.pref));
}
 
void build(node *n)
{
	if(n->tl == n->tr) {n->res.summ = a[n->tl]; n->res.pref = n->res.suff = n->res.ans = max(0LL, a[n->tl]);}
	else
	{
		int tm = (n->tl + n->tr)/2;	
...
{
	if(n->tl == n->tr) {n->res.summ = new_val; n->res.pref = n->res.suff = n->res.ans = max(0LL, new_val);}
	else
	{
		int tm = (n->tl + n->tr)/2;
...
int main()
{
	...
	while(t--)
	{
		...
		while(q--)
		{
			...
			update_query(default_node, k-1, x);
			cout<< default_node->res.ans <<nl;
		}
	}
